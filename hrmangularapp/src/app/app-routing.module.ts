...
const routes: Routes = [
  {path:'', component:MasterlayoutComponent,
    children:[
      ...
      {path:'interview', loadChildren:()=>import('./interview/interview.module').then(m=>m.InterviewModule), canActivate:[AuthGuard]},
      {path:'onboard', loadChildren:()=>import('./onboard/onboard.module').then(m=>m.OnboardModule), canActivate:[AuthGuard]},
      {path:'authen', loadChildren:()=>import('./authen/authen.module').then(m=>m.AuthenModule ), canActivate:[AuthGuard]}
    ]
  },
